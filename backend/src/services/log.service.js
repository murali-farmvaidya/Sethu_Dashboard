...
const { Log, Session } = require('../config/db');
const normalization = require('./pipecat_normalization');
...
class LogService {
    ...
    async bulkInsertLogs(agentId, agentName, sessionId, logs, startIndex = 0) {
        ...
        for (let i = 0; i < logs.length; i++) {
            ...
            // 3. Extract telephony metadata if present
            const telephonyMetadata = normalization.extractTelephonyMetadata(originalMessage);
            if (telephonyMetadata) {
                // Asynchronously update session metadata - don't wait to keep sync fast
                Session.updateOne(
                    { session_id: sessionId },
                    {
                        $set: {
                            'metadata.telephony': telephonyMetadata,
                            'metadata.transport': telephonyMetadata.transport || 'exotel',
                            'metadata.call_id': telephonyMetadata.call_id
                        }
                    }
                ).catch(err => logger.error(`Failed to update session telephony metadata: ${err.message}`));
            }
